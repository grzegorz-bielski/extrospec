  ...
  Effect.gen(function* () {
    ...
    return {
      ask: (message: string): ChatResponse =>
        Stream.fromEffect(Effect.tryPromise(() => chatEngine.chat({ message, stream: true }))).pipe(
          Stream.flatMap(a => Stream.fromAsyncIterable(a, e => new UnknownException(`Got an error ${e}`)))
        ),
    }
  })
)
